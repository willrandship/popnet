...
def forward(state,mask,inv):
	return mpz(mpz((state & mask)^inv).bit_count() > mask.bit_count())
...
def forward_layer(state, masks, invs):
	return mpz(sum(
		[forward(state, masks[x], invs[x]) << x for x in range(len(masks))]
	))

# reverse a node, given:
# * Ideal activation states
# * Inputs to forward (state, mask, inv)
#
# produce:
# * new mask, inv
# To do this
...
def reverse(ideal, state, mask, inv, s_in, seed=seed_zero):
	actual = forward_nothresh(state,mask,inv)
	precount = forward_nocount(state,mask,inv)
	result = mpz(actual>mask.bit_count())
...
#(fully convolutional)
#def reverse_layer(ideals, state, masks, invs, s_in):
def reverse_layer(ideals, state, masks, invs, s_in):
	masks_out = masks
	invs_out = invs
	ideals_out = mpz(0)
	...
	for x in range(len(masks)):
		masks_out[x],invs_out[x],wrongs[x] = reverse(
			ideals >> x,state,masks[x],invs[x],s_in
		)
	...
	#[mask_out, inv_out] = reverse(ideal, state, mask, inv)
...
class Layer:
	...
	def forward(self,state):
		return forward_layer(state,self.masks,self.invs)
	...
	def reverse(self,ideal,state):
		self.masks,self.invs,ideal_out = reverse_layer(
			ideal,state,self.masks,self.invs,self.s_in )
		return ideal_out
